...
def filter(signal_data, fs):
    ...
    uthresh = sigma * np.sqrt(2 * np.log(len(filtered_signal)))
    ct = list(coeffs)
    for i in range(1, len(coeffs)):
        ct[i] = pywt.threshold(coeffs[i], uthresh, mode='soft')
    output = pywt.waverec(ct, wavelet)
    
    return output[:len(signal_data)]
...
def threshold(Z, thr_percent=5.0, method='range'):
...
def mask_threshold(mask, time_index):
...
def plot_segment(cwt_data, time_axis, freqs, segments, labels, title, threshold_val):
    fig = go.Figure()
    gaitcycle = (time_axis/time_axis[-1]) * 100
...
foot_plot = go.Figure()
foot_plot.add_trace(go.Scatter(x=gaitcycle, y=RT_FOOT, name='PCG Signal', mode='lines', line=dict(color='rgba(177, 255, 0, 0.8)')))

latg_plot = go.Figure()
latv_plot = go.Figure()
latg_plot.add_trace(go.Scatter(x=gaitcycle, y=LAT_G, name='Raw', mode='lines', line=dict(color='rgba(255, 185, 33, 0.3)')))
latg_plot.add_trace(go.Scatter(x=gaitcycle, y=filt_LAT_G, name='Filtered', mode='lines', line=dict(color='rgba(185, 255, 33, 0.8)')))
latv_plot.add_trace(go.Scatter(x=gaitcycle, y=LAT_V, name='Raw', mode='lines', line=dict(color='rgba(33, 185, 255, 0.3)')))
latv_plot.add_trace(go.Scatter(x=gaitcycle, y=filt_LAT_V, name='Filtered', mode='lines', line=dict(color='rgba(33, 255, 185, 0.8)')))
...
# fix
handler.save(INDEX,filt_LAT_G, "filtered_LAT_G")
handler.save(INDEX,filt_LAT_V, "filtered_LAT_V")
handler.save(INDEX,RT_FOOT, "RT_FOOT")
...
if st.button("Apply CWT"):
    ...
    latg_cwt_plot.add_trace(go.Heatmap(
        z=latg_cwt, x=gaitcycle, y=cwt_freqs,colorscale='Jet', colorbar=dict(title='Magnitude')
    ))
    latg_cwt_plot.update_layout(
        title='Lateralis G Scalogram',xaxis_title='Gait Cycle (%)', yaxis_title='Frequency',yaxis=dict(type='linear'))


    latv_cwt_plot = go.Figure()
    latv_cwt_plot.add_trace(go.Heatmap(
        z=latv_cwt, x=gaitcycle, y=cwt_freqs,colorscale='Jet', colorbar=dict(title='Magnitude')
    ))
    latv_cwt_plot.update_layout(
        title='Lateralis V Scalogram',xaxis_title='Gait Cycle (%)', yaxis_title='Frequency',yaxis=dict(type='linear'))
    ...
    mask_GL, val_thr_GL = threshold(latg_cwt, thr_percent=thr_gl_pct, method='range')
    segs_GL_time = mask_threshold(mask_GL, gaitcycle)
    labels_GL = [label_gl(seg, gaitcycle) for seg in segs_GL_time]
    mask_VL, val_thr_VL = threshold(latv_cwt, thr_percent=thr_vl_pct, method='range')
    segs_VL_time = mask_threshold(mask_VL, gaitcycle)
    labels_VL = [label_vl(seg, gaitcycle) for seg in segs_VL_time]
    ...
    with col_cwt1:
        fig_seg_gl = plot_segment(
            latg_cwt, gaitcycle, cwt_freqs, segs_GL_time, labels_GL, "Lateralis G Activations", val_thr_GL
        )
    ...
    with col_cwt2:
        fig_seg_vl = plot_segment(
            latv_cwt, gaitcycle, cwt_freqs, segs_VL_time, labels_VL, "Lateralis V Activations", val_thr_VL
        )
    ...
    fig_lg_3d = plot_3d_cwt(latg_cwt, gaitcycle, cwt_freqs, "Lateralis G - 3D CWT")
    st.plotly_chart(fig_lg_3d, use_container_width=True)

    st.write("Lateralis V (3D Surface)")
    fig_lv_3d = plot_3d_cwt(latv_cwt, gaitcycle, cwt_freqs, "Lateralis V - 3D CWT")
