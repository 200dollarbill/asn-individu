...
class STFTConfigurator:
    """
    Calculates STFT parameters based on high-level user requirements
    and creates a configured STFT instance.
    """
    def __init__(self, signal_duration_seconds, sampling_rate, 
                 overlap_percentage, window_count, window_function):
        """
        Initializes the configurator with desired outcomes.

        Parameters
        ----------
        signal_duration_seconds : float
            The total duration of the signal you will be analyzing.
        sampling_rate : int
            The sampling rate of the signal (e.g., 44100).
        overlap_percentage : float
            The desired overlap between windows (e.g., 50 for 50%).
        window_count : int
            The desired number of windows (time frames) in the STFT output.
        window_function : scipy.signal.window object
        ...
        # --- Perform the inverse calculations ---
        self._calculate_parameters(overlap_percentage, window_count)

    def _calculate_parameters(self, overlap_percentage, window_count):
        """Derives the core STFT parameters."""
        overlap_decimal = overlap_percentage / 100.0

        # This is the core mathematical formula derived from the STFT process.
        # It solves for segment_length (L) based on the total samples (N),
        # window count (C), and overlap (p).
        # The formula is: L = N / ((C - 1) * (1 - p) + 1)
        ...
        # The shift_length is directly related to the segment_length and overlap
        shift_length_float = self.segment_length * (1 - overlap_decimal)
        self.shift_length = int(round(shift_length_float))
        
        # A good practice is to set the padded length to the next power of two
        # for efficient FFT calculations.
    ...
    def create_stft_instance(self):
        """
        Creates and returns a configured instance of the main STFT class.
        """
        return STFT_LIB(
